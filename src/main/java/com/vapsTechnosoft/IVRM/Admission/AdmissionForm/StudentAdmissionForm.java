...
public class StudentAdmissionForm extends TestBase {
	...
	private WebElement btn_DobCalender;

	@FindBy(xpath = "//span[contains(text(),'Dec 2014')]")
	WebElement btn_MonthYear;

	@FindBy(xpath = "//span[contains(text(),'Dec 2014')]/following::td[3]/span")
	WebElement btn_Date;
...
//		log("Clicked on age confirrmation pop up message.");
		...
		if(btn_DobCalender.isDisplayed()){
			try{
				btn_DobCalender.click();
				Thread.sleep(1000);
				((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);",btn_MonthYear);
				Thread.sleep(1000);
				btn_Date.click();
				log("DOB is selected for academic year.");
				Thread.sleep(1000);
			}
			catch (Exception e) {
				log("DOB not selected.");
				Thread.sleep(1000);
			}
		}
		else{
			log("DOB Calendar button element not present.");
			Thread.sleep(500);
		}
	//	calendarDatePicker(btn_DobCalender, btn_MonthYear, btn_Date);
